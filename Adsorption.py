...
def reorient_molecule(molecule, phi, theta):
    """
    Reorient the molecule to align with the calculated orientation.
...
def rotate_molecule_z(molecule, theta_z, origine):
    """
    Rotates the molecule arround z axis.
    
    Args:
        molecule (ASE Atoms object): The molecule structure.
        theta_z (float): Polar angle.
    """
    # Rotate molecule around z axes
    molecule.rotate(theta_z, 'z', center=(molecule[origine].x, molecule[origine].y, molecule[origine].z))
...
def main():
    ...
    parser.add_argument("--theta_z", type=float, help="Rotation angle arround z.")
    ...
    # Rotate molecule around z axes
    rotate_molecule_z(molecule, args.theta_z, args.origine)
